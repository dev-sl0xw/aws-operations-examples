...
AWS Organizations는 여러 AWS 어카운트를 일원 관리하는 서비스이다. 기업이 성장함에 따라 단일 AWS 어카운트에서의 운영은 한계에 도달한다. 환경 분리(개발/프로덕션), 팀 간 권한 분리, 청구 일원화, 보안 가드레일 적용 등 조직적인 클라우드 운영에는 멀티 어카운트 전략이 불가결하다. Organizations는 이 전략의 기반을 제공한다.
...
**정의:** AWS Organizations는 루트 어카운트(관리 어카운트)를 정점으로 하여, OU (Organizational Unit)에 의한 트리 구조로 멤버 어카운트를 계층적으로 관리한다.

**계층 구조의 예:**

```
Organization Root (관리 어카운트)
├── OU: Security
│   ├── Account: Log Archive (전체 어카운트의 로그 집약)
...
| 기능 | 설명 |
|------|------|
| **통합 청구 (Consolidated Billing)** | 전체 어카운트의 청구를 관리 어카운트로 통합. 볼륨 디스카운트의 혜택 |
| **SCP (Service Control Policies)** | OU/어카운트 단위로 권한의 상한을 설정 |
| **CloudFormation StackSets** | 여러 어카운트 및 리전에 일괄 배포 |
...
**정의:** SCP는 Organizations의 기능이며, OU(조직 단위) 또는 멤버 어카운트에 대해 사용 가능한 AWS 서비스와 액션의 상한을 설정하는 정책이다. SCP는 권한을 부여하지 않는다 -- 어디까지나 상한(가드레일)을 설정할 뿐이며, 실제 접근에는 IAM 정책이 필요하다.

**SCP의 중요한 성질:**

1. **관리 어카운트에는 적용되지 않는다:** 관리 어카운트에 SCP를 연결해도 효과가 없다. 이것이 관리 어카운트에서 워크로드를 실행해서는 안 되는 이유 중 하나.
2. **상속된다:** 상위 OU에 연결된 SCP는 하위 OU 및 하위 어카운트에 자동으로 상속된다.
...
> A: IAM 정책은 개별 사용자나 역할에 대해 권한을 설정하지만, 어카운트 전체의 가드레일로서는 기능하지 않는다. 예를 들어 어카운트 관리자가 IAM 정책을 자유롭게 변경할 수 있는 환경에서는, 관리자가 자기 자신에게 AdministratorAccess를 부여하여 무엇이든 할 수 있게 된다. SCP는 개별 IAM 정책의 상위에 위치하여, "이 어카운트에서는 어떤 IAM 정책을 설정해도 이 조작만은 절대로 할 수 없다"라는 하드 리밋을 설정한다.
> Q: "SCP가 관리 어카운트에 적용되지 않는 이유는?"
> A: 관리 어카운트에 SCP가 적용되면 조직 전체의 락아웃이 발생할 위험이 있다. 관리 어카운트는 Organizations의 관리 조작을 수행하는 특권 어카운트이며, 만일의 설정 실수로부터의 복구에 필요하다. 그래서 관리 어카운트에서는 워크로드를 실행하지 않고 관리 목적으로만 사용해야 한다.

**현실 세계의 비유 (비IT 대상):**
> "건물의 소방법과 같은 것. 각 임차인(어카운트)은 방을 마음대로 장식할 수 있지만(IAM 정책), 비상구는 막을 수 없습니다(SCP 가드레일). 소방법은 건물 전체에 적용되며, 개별 임차인의 의향과 관계없이 준수가 필요합니다. 다만 건물 소유주(관리 어카운트)는 소방법 적용 대상 외 -- 그래서 건물 소유주의 방에는 특별한 주의가 필요합니다"

---

### OU (Organizational Unit)

**정의:** OU는 Organizations 내에서 어카운트를 그룹화하는 논리적 컨테이너. 최대 5계층까지 중첩 가능. OU에 SCP를 연결하여 하위의 전체 어카운트에 정책을 일괄 적용할 수 있다.

**소크라테스식 심화:**
> Q: "왜 OU 구조가 중요한가? 전체 어카운트를 플랫하게 나열하면 안 되는가?"
> A: 플랫 구조에서는 어카운트별로 개별적으로 SCP를 관리해야 하며, 어카운트 수가 증가하면 스케일하지 않는다. OU 구조를 사용하면 "프로덕션 환경 OU"에 연결한 SCP가 하위의 전체 프로덕션 어카운트에 자동 적용된다. 100개의 프로덕션 어카운트가 있어도 SCP 관리는 1곳에서 충분하다.
> Q: "OU 구조를 설계할 때의 지침은?"
...
**현실 세계의 비유 (비IT 대상):**
> "회사의 조직도와 같은 것. 경영진(root) 아래에 부서(OU)가 있고, 부서 아래에 팀(어카운트)이 있습니다. 부서 규칙은 자동으로 팀 전체에 적용됩니다. 인사부의 규칙(예: 근태 관리 방법)은 인사부의 전체 팀에 적용되며, 엔지니어링 부서에는 영향을 미치지 않습니다"
...
| `root-account-mfa-enabled` | 루트 어카운트에 MFA가 활성화되어 있는가 |
...
**정의:** AWS Config Aggregator는 여러 AWS 어카운트 및 리전으로부터 Config의 구성 데이터와 컴플라이언스 결과를 집약하는 리소스. 조직 전체의 컴플라이언스 상황을 일원적으로 파악할 수 있다.

**2가지 어그리게이터 소스:**

| 소스 유형 | 설명 |
|-------------|------|
| **Organizations** | 조직 내의 전체 어카운트 및 전 리전으로부터 자동 집약 |
| **개별 어카운트** | 지정한 어카운트 ID와 리전으로부터 집약 (조직 외 어카운트도 가능) |

**구성:**

```
집약 어카운트 (Aggregator Account)
  ├── Config Aggregator
  │     ├── 소스: Organization (전체 멤버 어카운트)
  │     └── 리전: 전 리전
  │
  ├── 집약 뷰: 전체 어카운트의 비준수 리소스 목록
  └── 대시보드: 컴플라이언스 요약
```
...
AWS Control Tower는 베스트 프랙티스에 기반한 멀티 어카운트 환경을 자동 설정한다. 필수 가드레일(SCP + Config Rules)이 자동으로 적용된다.
...
Log Archive Account
  ├── CloudTrail 로그 (전체 어카운트)
  ├── Config 스냅샷 (전체 어카운트)
  ├── VPC Flow Logs (전체 어카운트)
  └── S3 버킷 (WORM: Write Once Read Many)
```

보안 도구와 감사 로그를 전용 어카운트에 분리하여, 워크로드 어카운트의 관리자가 로그를 변조할 수 없게 한다.
...
- **SCP는 관리 어카운트에 적용되지 않는다.** 관리 어카운트의 IAM 사용자는 SCP의 제약을 받지 않는다.
...
- **Conformance Packs는 조직 수준에서 배포 가능**하며, 전체 멤버 어카운트에 일괄 적용할 수 있다.
- **Organizations의 통합 청구**에서는 전체 어카운트의 리소스 사용량이 합산되므로, 볼륨 디스카운트(S3, EC2 RI 등)의 혜택을 받을 수 있다.
...
### Security
- [ ] 멀티 어카운트 전략이 수립되어 OU 구조가 설계되어 있는가
- [ ] 관리 어카운트에서 워크로드를 실행하고 있지 않은가
- [ ] SCP로 리전 제한이 적용되어 있는가 (불필요한 리전에서의 리소스 생성 금지)
- [ ] SCP로 CloudTrail/Config 비활성화가 금지되어 있는가
- [ ] AWS Config가 전체 어카운트 및 전 리전에서 활성화되어 있는가
- [ ] Config Aggregator로 조직 전체의 컴플라이언스가 가시화되어 있는가
- [ ] 비준수 리소스에 대한 수정 액션이 정의되어 있는가
- [ ] 보안 로그가 전용 어카운트에 집약되어 변조 방지되어 있는가

### Operational Excellence
- [ ] 신규 어카운트 생성이 자동화되어 있는가 (Account Factory)
